...
for i in tqdm(range(args.eval_episodes)):
    ...
    evaluation_metrics.append({
        ...
        # ---- 새로 추가: make_plan 전체/ DINO / SAM 평균시간(초) ----
        'planner_avg_time_sec': float(np.mean(nav_planner.planner_durations)) if len(nav_planner.planner_durations) > 0 else 0.0,
        'dino_avg_time_sec': float(np.mean(nav_planner.dino_durations)) if len(nav_planner.dino_durations) > 0 else 0.0,
        'sam_avg_time_sec': float(np.mean(nav_planner.sam_durations)) if len(nav_planner.sam_durations) > 0 else 0.0,
    })
